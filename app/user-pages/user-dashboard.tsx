...
export default function AccountDashboard() {
  ...
  const handleUpload = async () => {
    ...
    setUploading(true);
    // Demo-only: pretend upload succeeded.
    await new Promise((r) => setTimeout(r, 600));
    setUploading(false);
    window.alert("Profile picture uploaded (demo only).");
  };

  if (!user) return <p className="text-center text-muted-foreground">No user data found.</p>;
    ...
    switch (activeTab) {
      ...
      case "profile":
        return (
           <div className="space-y-6">
            <div className="text-center mb-8">
              <h2 className="text-3xl font-bold text-primary mb-2">User Details</h2>
            </div>
            ...
            <div className="max-w-2xl mx-auto bg-card p-8 rounded-lg border">
              <div className="flex items-center mb-6">
                <div className="w-20 h-20 bg-primary rounded-full flex items-center justify-center mr-6">
                  <span className="text-primary-foreground text-2xl font-bold">{user.avatarInitial || user.user_name?.[0] || "?"}</span>
                </div>
